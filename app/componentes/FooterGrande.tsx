    ...
    <footer className="mt-auto bg-gray-950 text-gray-400">
      ...
      <div className="max-w-7xl mx-auto px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <p className="flex items-center gap-2">
                <svg className="w-4 h-4 text-orange-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                info@truckwave.com
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              {[['home', '/home'], ['services', '/servicios'], ['about', '/sobre-nosotros'], ['contact', '/contacto']].map(([key, href]) => (
                <li key={key}>
                  <Link href={href} className="flex items-center gap-2 hover:text-orange-400 transition-colors group">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              {[['nationalTransport'], ['internationalTransport'], ['logistics'], ['storage']].map(([key]) => (
                <li key={key}>
                  <Link href="#" className="flex items-center gap-2 hover:text-orange-400 transition-colors group">
                    <span className="w-1.5 h-1.5 rounded-full bg-orange-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                    {t(key)}
                  </Link>
                </li>
        ...
        <div className="mt-12 pt-6 border-t border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-600">
          <p>© {new Date().getFullYear()} TruckWave. Todos los derechos reservados.</p>
