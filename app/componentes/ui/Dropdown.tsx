  ...
  return (
    <div>
      ...
      <div className="relative">
        <button
          onClick={onToggle}
          className="flex items-center gap-3 px-4 py-3 border-2 border-border/30 rounded-xl bg-white text-sm justify-between hover:border-primary/50 transition-all"
          style={{ minWidth }}
        >
          ...
          <div className="absolute z-10 mt-2 bg-white border-2 border-border/30 rounded-xl shadow-2xl w-full overflow-hidden">
            {options.map((op) => (
              <div
                key={op}
                onClick={() => onSelect(op)}
                className="px-4 py-3 text-sm hover:bg-primary/10 hover:text-primary cursor-pointer text-left transition-colors border-b border-border/10 last:border-b-0"
              >
                {op}
              </div>
            ))}
